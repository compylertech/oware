    ...
    <LoansShell>
      <div className="grid gap-4" style={{ gridTemplateColumns: "1.5fr 1fr" }}>
        <Panel>
          ...
          <div className="p-5 space-y-4">
            <div
              ...
              style={{
                padding: 12, background: "#F4F6FB", borderRadius: 10,
                border: `1px solid ${LOAN.border}`,
              }}
            >
              <Ava name="Adwoa Mensa" bg="#DC2626" size={36} />
              <div>
                <div style={{ ...fontMono, fontSize: 12, fontWeight: 700, color: LOAN.navy }}>LN-20142</div>
                ...
                {(["MoMo", "Bank", "Cash"] as const).map((m) => (
                  <button
                    ...
                    style={{
                      ...
                      color: LOAN.ink, fontSize: 12, fontWeight: 600,
                    }}
                  >
                    {m}
                  </button>
            ...
            <div style={{ background: "#F8FAFD", borderRadius: 10, padding: 14, border: `1px solid ${LOAN.border}` }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: LOAN.muted, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>
                Allocation preview
              </div>
              {[
                ["Penalty", 80], ["Interest", 210], ["Fees", 35], ["Principal", 875],
              ].map(([l, v]) => (
                <div key={l as string} className="flex justify-between" style={{ fontSize: 12, padding: "4px 0", color: LOAN.ink }}>
                  <span>{l}</span><span>{fmtGHS(v as number)}</span>
                </div>
              ))}
              <div style={{ borderTop: `1px dashed ${LOAN.border}`, marginTop: 6, paddingTop: 8 }} className="flex justify-between">
                <span style={{ fontSize: 12, fontWeight: 700, color: LOAN.ink }}>Total</span>
                <span style={{ ...fontDisplay, fontSize: 16, fontWeight: 800, color: LOAN.ink }}>{fmtGHS(1200)}</span>
              </div>
            </div>
        ...
        <Panel>
          ...
          <div>
            {RECENT.map((r, i) => (
              <div key={i} className="flex items-center gap-3" style={{ padding: "12px 16px", borderTop: i === 0 ? "none" : `1px solid ${LOAN.border}` }}>
                <Ava name={r.name} bg={r.avatar} size={32} />
                <div className="flex-1 min-w-0">
                  <div style={{ fontSize: 13, fontWeight: 600, color: LOAN.ink }}>{r.name}</div>
                  <div style={{ fontSize: 11, color: LOAN.muted }}>{r.method} · {r.date}</div>
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: LOAN.ink }}>{fmtGHS(r.amount)}</div>
              </div>
            ))}
          </div>
        </Panel>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 600, color: LOAN.muted, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 4 }}>{label}</div>
      {children}
    </div>
  );
}
