      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "1.4fr 1fr" }}>
        <Panel>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr><Th>Client</Th><Th>Outstanding</Th><Th>Overdue</Th><Th>Bucket</Th></tr>
            </thead>
            <tbody>
              {OVERDUE.map((o) => (
                <tr key={o.client}>
                  ...
                  <Td><StagePill stage={o.bucket} /></Td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Aging({ label, amt, pct, color }: { label: string; amt: string; pct: number; color: string }) {
  return (
    <div>
      <div className="flex justify-between" style={{ fontSize: 12, color: LOAN.ink, marginBottom: 4 }}>
        <span>{label}</span><span style={{ color: LOAN.muted }}>{amt} · {pct}%</span>
      </div>
