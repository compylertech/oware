...
const ROWS: Row[] = [
  { acct: "ACC-00001", client: "Kwame Asante", type: "Credit", amount: 100000, narration: "Salary deposit", status: "Completed", date: "22 Jun 2026" },
  { acct: "ACC-00001", client: "Kwame Asante", type: "Debit", amount: 800, narration: "Utility payment", status: "Completed", date: "22 Jun 2026" },
  { acct: "ACC-00002", client: "Abena Mensah", type: "Debit", amount: 4000, narration: "Loan repayment", status: "Completed", date: "20 Jun 2026" },
  { acct: "ACC-00003", client: "Kofi Boateng", type: "Credit", amount: 12500, narration: "Business income", status: "Completed", date: "18 Jun 2026" },
  { acct: "ACC-00002", client: "Abena Mensah", type: "Debit", amount: 200, narration: "Mobile transfer", status: "Completed", date: "15 Jun 2026" },
  { acct: "ACC-00004", client: "Ama Darko", type: "Credit", amount: 39.8, narration: "Interest earned", status: "Completed", date: "12 Jun 2026" },
  { acct: "ACC-00005", client: "Yaw Owusu", type: "Credit", amount: 5000, narration: "Cash deposit", status: "Completed", date: "10 Jun 2026" },
  { acct: "ACC-00003", client: "Kofi Boateng", type: "Debit", amount: 1200, narration: "Insurance premium", status: "Completed", date: "08 Jun 2026" },
  { acct: "ACC-00006", client: "Efua Tetteh", type: "Credit", amount: 750, narration: "Refund", status: "Pending", date: "05 Jun 2026" },
  { acct: "ACC-00001", client: "Kwame Asante", type: "Debit", amount: 3500, narration: "Withdrawal", status: "Completed", date: "01 Jun 2026" },
];
...
function showToast(msg: string) {
  ...
  Object.assign(el.style, {
    ...
    background: "#16233F", color: "#fff", padding: "10px 16px",
  ...
  return (
    <div style={{ background: BG, minHeight: "100%", padding: "24px 28px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 16 }}>
        ...
        <Kpi label="TOTAL CREDITS" value={`GHS ${fmt(118289.8)}`} valueColor="#067647" sub="Completed credits" />
        <Kpi label="TOTAL DEBITS" value={`GHS ${fmt(9700)}`} valueColor="#D92D20" sub="Completed debits" />
      ...
      <Card style={{ padding: 16, marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8, border: `1px solid ${BORDER}`, borderRadius: 10, padding: "8px 12px" }}>
          ...
          <thead>
            <tr style={{ borderBottom: `2px solid ${NAVY}` }}>
              {["ACCOUNT NO.", "CLIENT", "TYPE", "AMOUNT", "NARRATION", "STATUS", "DATE", ""].map((h) => (
                <th key={h} style={{ textAlign: "left", padding: "14px 18px", fontSize: 12, fontWeight: 700, color: MUTED, textTransform: "uppercase", letterSpacing: "0.08em" }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {ROWS.map((r, i) => (
              <tr key={i} style={{ borderBottom: i < ROWS.length - 1 ? `1px solid ${BORDER}` : "none" }}>
                <td style={td}>
                  <span style={{ fontFamily: "DM Mono, monospace", color: NAVY, fontSize: 13 }}>{r.acct}</span>
                </td>
                <td style={td}>
                  ...
                  {openMenu === i && (
                    <div
                      ...
                      style={{
                        ...
                        padding: 4, minWidth: 170, textAlign: "left",
                      }}
                    >
                      <MenuItem onClick={() => { setOpenMenu(null); setDetail(r); }}>View details</MenuItem>
                      <MenuItem onClick={() => { setOpenMenu(null); showToast("Receipt downloaded"); }}>Download receipt</MenuItem>
                      <MenuItem onClick={() => { setOpenMenu(null); showToast("Transaction flagged for review"); }}>Flag transaction</MenuItem>
                    </div>
                  )}
                </td>
              </tr>
      ...
      {detail && (
        ...
        >
          <div
            ...
            style={{ width: 380, height: "100%", background: "#fff", padding: 24, overflow: "auto" }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h3 style={{ fontFamily: "Sora, sans-serif", fontSize: 18, fontWeight: 700, color: INK, margin: 0 }}>Transaction details</h3>
              <button onClick={() => setDetail(null)} style={{ background: "transparent", border: "none", cursor: "pointer", color: MUTED, fontSize: 18 }}>×</button>
            </div>
            ...
            <DetailRow label="Amount" value={`${detail.type === "Credit" ? "+" : "-"}GHS ${fmt(detail.amount)}`} />
...
function DetailRow({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0", borderBottom: `1px solid ${BORDER}`, fontSize: 13 }}>
      ...
      <span style={{ color: INK, fontWeight: 600, fontFamily: mono ? "DM Mono, monospace" : "DM Sans, sans-serif" }}>{value}</span>
    </div>
  );
}
...
function Kpi({ label, value, valueColor, sub }: { label: string; value: string; valueColor: string; sub: string }) {
  return (
    <Card style={{ padding: "20px 22px" }}>
      ...
      <div style={{ fontFamily: "Sora, sans-serif", fontSize: 28, fontWeight: 800, color: valueColor, marginTop: 6, fontVariantNumeric: "tabular-nums" }}>{value}</div>
  ...
  return (
    <button
      ...
      style={{
        ...
        color: INK, fontSize: 13, fontWeight: 600, lineHeight: 1.2,
        borderRadius: 8, padding: "8px 16px", height: 36, cursor: "pointer",
      }}
    >
      {icon}
      {children}
  ...
  return (
    <button
      ...
      style={{
        ...
        background: "#fff", border: `1px solid ${BORDER}`,
        color: INK, fontSize: 13, fontWeight: 600, lineHeight: 1.2,
        borderRadius: 8, padding: "8px 12px", height: 36, cursor: "pointer",
      }}
    >
      {label}
